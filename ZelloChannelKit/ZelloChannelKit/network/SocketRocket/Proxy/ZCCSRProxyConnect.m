...
@interface ZCCSRProxyConnect() <NSStreamDelegate>
...
@implementation ZCCSRProxyConnect
{
    ZCCSRProxyConnectCompletion _completion;
...
-(instancetype)initWithURL:(NSURL *)url
{
    ...
    _url = url;
    _connectionRequiresSSL = ZCCSRURLRequiresSSL(url);
...
- (void)dealloc
{
    ...
    [self.inputStream removeFromRunLoop:[NSRunLoop ZCCSR_networkRunLoop] forMode:NSDefaultRunLoopMode];
...
- (void)openNetworkStreamWithCompletion:(ZCCSRProxyConnectCompletion)completion
{
...
- (void)_didConnect
{
    ZCCSRDebugLog(@"_didConnect, return streams");
    if (_connectionRequiresSSL) {
        if (_httpProxyHost) {
            // Must set the real peer name before turning on SSL
            ZCCSRDebugLog(@"proxy set peer name to real host %@", self.url.host);
    ...
    [inputStream removeFromRunLoop:[NSRunLoop ZCCSR_networkRunLoop] forMode:NSDefaultRunLoopMode];
...
{
    ZCCSRDebugLog(@"_failWithError, return error");
    if (!error) {
        error = ZCCSRHTTPErrorWithCodeDescription(500, 2132,@"Proxy Error");
    }
    ...
    [self.inputStream removeFromRunLoop:[NSRunLoop ZCCSR_networkRunLoop]
...
- (void)_configureProxy
{
    ...
    if (proxies.count == 0) {
        ZCCSRDebugLog(@"configureProxy no proxies");
...
- (void)_readProxySettingWithType:(NSString *)proxyType settings:(NSDictionary *)settings
{
    ...
    if (_httpProxyHost) {
        ZCCSRDebugLog(@"Using http proxy %@:%u", _httpProxyHost, _httpProxyPort);
    } else if (_socksProxyHost) {
        ZCCSRDebugLog(@"Using socks proxy %@:%u", _socksProxyHost, _socksProxyPort);
    } else {
        ZCCSRDebugLog(@"configureProxy no proxies");
    }
}
...
{
    ZCCSRDebugLog(@"ZCCSRWebSocket fetchPAC:%@", PACurl);
...
- (void)_openConnection
{
    ...
    [self.inputStream scheduleInRunLoop:[NSRunLoop ZCCSR_networkRunLoop]
                                forMode:NSDefaultRunLoopMode];
...
- (void)_initializeStreams
{
    ...
    ZCCSRDebugLog(@"ProxyConnect connect stream to %@:%u", host, port);
    ...
    if (_socksProxyHost) {
        ZCCSRDebugLog(@"ProxyConnect set sock property stream to %@:%u user %@ password %@", _socksProxyHost, _socksProxyPort, _socksProxyUsername, _socksProxyPassword);
...
{
    ZCCSRDebugLog(@"stream handleEvent %u", eventCode);
    switch (eventCode) {
...
- (void)_proxyDidConnect
{
    ...
    ZCCSRDebugLog(@"Proxy sending %@", connectRequestStr);
...
{
    NSMutableData *buf = [NSMutableData dataWithCapacity:ZCCSRDefaultBufferSize()];
    uint8_t *buffer = buf.mutableBytes;
    NSInteger length = [_inputStream read:buffer maxLength:ZCCSRDefaultBufferSize()];
...
- (void)_proxyHTTPHeadersDidFinish
{
    ...
    if (responseCode >= 299) {
        ZCCSRDebugLog(@"Connect to Proxy Request failed with response code %d", responseCode);
    ...
    ZCCSRDebugLog(@"proxy connect return %d, call socket connect", responseCode);
    [self _didConnect];
}
...
- (void)_writeData:(NSData *)data
{
    ...
    dispatch_async(_writeQueue, ^{
        ...
        while (![outStream hasSpaceAvailable]) {
            ...
            timeout -= 100;
            if (timeout < 0) {
                NSError *error = ZCCSRHTTPErrorWithCodeDescription(408, 2132, @"Proxy timeout");
